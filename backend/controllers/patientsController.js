...
function parsePatientIdSeed(input) {
  ...
  const seq = Number(digits.slice(6));
...
router.post("/id-counter", async (req, res) => {
  try {
    ...
    const body = req.body || {};
    const seedFromPatientId = parsePatientIdSeed(body.patient_id || body.patientId);

    let ym = seedFromPatientId?.ym || String(body.ym || body.yearMonth || "").trim();
    let lastSeq = seedFromPatientId?.seq ?? Number(body.last_seq ?? body.lastSeq);

    if (!ym || !/^\d{6}$/.test(ym) || Number.isNaN(lastSeq)) {
      return res.status(400).json({
        success: false,
        message: "Provide { patient_id } or { ym, last_seq }",
      });
    }
    ...
    return res.json({
      success: true,
      message: "Patient ID counter updated",
      counter: result.rows[0],
    });
  } catch (err) {
    console.error("Patient ID counter update error:", err.message);
    return res.status(500).json({
      success: false,
      message: "Failed to update patient id counter",
      error: err.message,
    });
  }
});
