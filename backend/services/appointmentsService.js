...
async function listAppointments(date) {
  const columns = await getAppointmentColumns();
...
async function saveAppointment(body) {
  ...
  if (!patientIdCol) {
    const err = new Error("appointments table is missing patient id column");
    err.status = 500;
    throw err;
  }
  ...
  return { appointment: mapAppointmentRow(saved), mwl_updated: mwlUpdated };
}
...
async function updateAppointment(id, body) {
  ...
  const patientIdCol = ["patient_id", "patientid", "uhid"].find((c) => columns.has(c));
  if (!idCol) {
    const err = new Error("appointments table is missing id column");
    err.status = 500;
    throw err;
  }
  ...
  return { appointment: mapAppointmentRow(updated.rows[0]), mwl_updated: mwlUpdated };
}
